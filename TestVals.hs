...
env = newScope $
    ...
    -- { loadedGlobalTypes =
        Map.fromList
        [ ("fix",    forAll 1 0 $ \ [a] [] -> (a ~> a) ~> a)
        , ("if",     forAll 1 0 $ \ [a] [] -> recordType [("condition", boolType), ("then", a), ("else", a)] ~> a)
        , ("==",     forAll 1 0 $ \ [a] [] -> infixType a a boolType)
        , (">",      forAll 1 0 $ \ [a] [] -> infixType a a boolType)
        ...
        -- , (":",      forAll 1 0 $ \ [a] [] -> recordType [("head", a), ("tail", listOf a)] ~> listOf a)
        -- , ("[]",     forAll 1 0 $ \ [a] [] -> listOf a)
        -- , ("concat", forAll 1 0 $ \ [a] [] -> listOf (listOf a) ~> listOf a)
        -- , ("map",    forAll ["a", "b"] $ \ [a, b] [] -> recordType [("list", listOf a), ("mapping", a ~> b)] ~> listOf b)
        -- , ("..",     forAll 0 0 $ \ [] [] -> infixType intType intType (listOf intType))
        , ("||",     forAll 0 0 $ \ [] [] -> infixType boolType boolType boolType)
        -- , ("head",   forAll 1 0 $ \ [a] [] -> listOf a ~> a)
        , ("negate", forAll 1 0 $ \ [a] [] -> a ~> a)
        , ("sqrt",   forAll 1 0 $ \ [a] [] -> a ~> a)
        , ("id",     forAll 1 0 $ \ [a] [] -> a ~> a)
        -- , ("zipWith",forAll ["a","b","c"] $ \ [a,b,c] [] ->
                                  -- (a ~> b ~> c) ~> listOf a ~> listOf b ~> listOf c )
        -- , ("Just",   forAll 1 0 $ \ [a] [] -> a ~> maybeOf a)
        -- , ("Nothing",forAll 1 0 $ \ [a] [] -> maybeOf a)
        -- , ("maybe",  forAll ["a", "b"] $ \ [a, b] [] -> b ~> (a ~> b) ~> maybeOf a ~> b)
        , ("plus1",  forAll 0 0 $ \ [] [] -> intType ~> intType)
        -- , ("True",   forAll 0 0 $ \ [] [] -> boolType)
        -- , ("False",  forAll 0 0 $ \ [] [] -> boolType)
        ]
    -- , loadedNominals =
    --     Map.fromList
    --     [ listTypePair
